...
UploadCsv::UploadCsv(DefaultIO *dio, CommonData *commonData) : Command(dio, commonData,
                                                                       "upload an unclassified csv data file") {}

void UploadCsv::execute() {
    dio->write("Please upload your local train CSV file.\n");
    this->fileUpload("train_" + to_string(dio->socketID) + "dataset.csv");
    commonData->train_path = "train_" + to_string(dio->socketID) + "dataset.csv";
    dio->write("Please upload your local test CSV file.\n");
    this->fileUpload("test_" + to_string(dio->socketID) + "dataset.csv");
    commonData->test_path = "test_" + to_string(dio->socketID) + "dataset.csv";
...
void AlgSettings::execute() {
    ...
    while (flag) {
        // send current params
        stringstream ss;
        ss << "The current KNN parameters are: K = " << to_string(commonData->k) << ", distance mertic = "
           << commonData->metric << "\n";
        dio->write(ss.str());
        // get new params
        newParams = dio->read();
